...
code = code.replace("const app = express();", line_init)
        ...
        if (text === '/rc') {
            ...
            for (const userOwe in debts) {
                ...
                bubbles.push({
                    ...
                    body: {
                        type: 'box', layout: 'vertical', contents: [
                            { type: 'text', text: ${amount.toLocaleString()}d, weight: 'bold', size: 'sm', color: '#ff0000', align: 'center' }
                        ]
                    }
                });
            }
            ...
            if (menu) {
                ...
                for (let i = 0; i < Math.min(menu.items.length, 12); i++) {
                    ...
                    buttons.push({
                        ...
                        margin: 'sm',
                        height: 'sm',
                        action: { type: 'postback', label: btnText.substring(0, 20), data: i__, displayText: Ðã ch?n:  }
                    });
                }
...
code = code.replace("app.use(express.json());", line_webhook)
